...
test('Test Login, Articles et commentaires', async ({ page }) => {
  ...
  await expect(page.locator('div').filter({ hasText: 'Titre article 5' }).first()).toBeVisible();
  await expect(page.getByText('Titre article 5')).toBeVisible();
  await expect(page.getByText('Date 6/18/25, 1:45 AM Auteur').first()).toBeVisible();
  await expect(page.getByText('Content: lorem ipsum is').first()).toBeVisible();
  await expect(page.getByText('Titre article 1')).toBeVisible();
  await expect(page.getByText('Date 6/15/25, 11:54 AM Auteur')).toBeVisible();
  await expect(page.getByText('Content: lorem ipsum is').nth(4)).toBeVisible();
  await page.getByRole('button', { name: 'Inverser l\'ordre' }).click();
  await expect(page.getByText('Titre article 1')).toBeVisible();
  await expect(page.getByText('Date 6/15/25, 11:54 AM Auteur')).toBeVisible();
  await expect(page.getByText('Content: lorem ipsum is').first()).toBeVisible();
  await expect(page.getByText('Titre article 5')).toBeVisible();
  await expect(page.getByText('Date 6/18/25, 1:45 AM Auteur').nth(2)).toBeVisible();
  await expect(page.getByText('Content: lorem ipsum is').nth(4)).toBeVisible();
  await page.getByRole('button', { name: 'Inverser l\'ordre' }).click();
  await expect(page.getByText('Titre article 5')).toBeVisible();
  await expect(page.getByText('Titre article 5')).toBeVisible();
  ...
  await page.getByLabel('Thème :').selectOption('1');
